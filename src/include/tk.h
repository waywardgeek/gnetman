 ...
 *                              0 means unmapped.
  TK_TOP_LEVEL:         1 means this is a bottom-level window (it
...
typedef struct Tk_Item  {
    ...
    struct Tk_Item *nextPtr;            /* Next in display list of all
                                         ...
                                         * in list are drawn on bottom of earlier
                                         * ones. */
...
/*--------------------------------------------------------------------------------------------------
  The following structure describes a netlist of pixels in memory:
--------------------------------------------------------------------------------------------------*/

typedef struct Tk_PhotoImageNetlist {
    unsigned char *pixelPtr;    /* Pointer to the first pixel. */
    int         width;          /* Width of netlist, in pixels. */
    int         height;         /* Height of netlist, in pixels. */
...
typedef int (Tk_ImageFileWriteProc) _ANSI_ARGS_((Tcl_Interp *interp,
        char *fileName, char *formatString, Tk_PhotoImageNetlist *netlistPtr));
typedef int (Tk_ImageStringWriteProc) _ANSI_ARGS_((Tcl_Interp *interp,
        Tcl_DString *dataPtr, char *formatString,
        Tk_PhotoImageNetlist *netlistPtr));
...
EXTERN void             Tk_PhotoPutNetlist _ANSI_ARGS_((Tk_PhotoHandle handle,
                            Tk_PhotoImageNetlist *netlistPtr, int x, int y,
                            int width, int height));
EXTERN void             Tk_PhotoPutZoomedNetlist _ANSI_ARGS_((
